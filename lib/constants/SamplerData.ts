...
/** Omit payload fields when numeric value matches the sampler no-op default (#556). */
export function shouldOmitSamplerNumberValue(samplerID: SamplerID, value: unknown): boolean {
    if (typeof value !== 'number') return false
    const values = Samplers[samplerID].values
    if (values.type !== 'integer' && values.type !== 'float') return false
    return value === values.ignoreIf
}
